...
async fn main() -> SiwxResult<()> {
    ...
    // Example 1: Ethereum Public Key (only when feature enabled)
    #[cfg(feature = "ethereum")]
    {
        ...
        println!(
            "   Supports EIP-191: {}",
            eth_public_key.supports_signature_type(&SignatureType::Eip191)
        );
        println!(
            "   Supports Ed25519: {}",
            eth_public_key.supports_signature_type(&SignatureType::Ed25519)
        );
        println!();
    }
    ...
    #[cfg(feature = "solana")]
    {
        ...
        println!(
            "   Supports EIP-191: {}",
            sol_public_key.supports_signature_type(&SignatureType::Eip191)
        );
        println!(
            "   Supports Ed25519: {}",
            sol_public_key.supports_signature_type(&SignatureType::Ed25519)
        );
        println!();
    }
    ...
    // Example 4: Using with verifier (Ethereum only)
    #[cfg(feature = "ethereum")]
    {
    ...
    {
        let sol_for_chain =
            PublicKeyFactory::for_chain("11111111111111111111111111111112", Chain::Solana)?;
